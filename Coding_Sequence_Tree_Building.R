...
adam7DNA <- read.csv("~/Desktop/R/ADAM7_phylip_sequential.dna", row.names=1, sep="")
#view dataset
View(adam7DNA)
...
View(adam7mt)
# choose model based on test of choice, I went based on lowest loglikelihood score
...
adamtreeNJ <- (adam7dm)
# Plot UPGMA and NJ tree
# plotting parameters to show both trees on same page, omit layout() if want one at a time
layout(matrix(c(1,2), 2, 1), height=c(1,2))
cophyloplot(adamtreeUPGMA,adamtreeNJ, space = 5, font = 1)
